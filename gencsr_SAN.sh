...
create_csr()
 {
     expect -c "
         spawn openssl req -newkey rsa:${KEYLENGTH} -days ${DAYS} -key ${PRIVATEKEYPATH} -config openssl.cnf -out ${CSRPATH} -text
...
#  証明書の生成
openssl x509 -days ${DAYS} -req v3_req -extfile openssl.cnf -signkey ${PRIVATEKEYPATH} < ${CSRPATH} > ${PUBKEYPATH}
